...
func TestWebhookAPIKeys_Create(t *testing.T) {
	...
	assert.True(t, key.KeyValue.Valid, "raw key should remain visible to administrators")
	assert.True(t, key.Authenticate(key.KeyValue.String), "stored raw key must authenticate")
